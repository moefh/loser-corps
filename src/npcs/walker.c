...
void move_enemy(SERVER *server, ENEMY *e, NPC_INFO *npc)
{
  ...
  if(e->state == STATE_STAND || e->state == STATE_WALK)
  {
    ...
    else
    {
      ...
      if(e->dir == DIR_RIGHT) e->dx = ENEMY_WALK_SPEED;
      else e->dx = -ENEMY_WALK_SPEED;

      calc_movement(server, 0, e->x + e->dx/1000, e->y, 
		    npc->clip_w, npc->clip_h, 0, 1, &move_dx, &move_dy);

      if(move_dy > 0)
      {
	e->dx *= -1;
	if(e->dir == DIR_RIGHT) e->dir = DIR_LEFT;
